...
class TestSpiFlash(Module):
    def __init__(self):
        pads = Record([("clk", 1), ("cs_n", 1), ("mosi", 1), ("miso", 1)])
        ram_pad = Record([("ce_n", 1)])
        self.submodules.spiflash = SpiFlash(pads, with_bitbang=False, dummy=1, ram_pad=ram_pad, flashsize=0x100000 )

def testwrite(dut):
    print("Testing write...")
    for i in range(8):
        yield
    yield dut.spiflash.bus.write(0x1234, 0xdeadbeef, sel=0b1001)
    yield
...
def testread(dut):
    ...
    for i in range(8):
        yield
    data = yield from dut.spiflash.bus.read(0xFF00)
    yield
    data += yield from dut.spiflash.bus.read(0x100000)
    yield
    print(data)
...
if __name__ == "__main__":
    ...
    run_simulation(dut, gens, clocks={"sys": int(1000/21)}, vcd_name="test_spiflash_write.vcd")
