...
# This was modify for the project POGOBOT
    ...
    def upload(self, filename, address):
        ...
        while remaining:
            ...
            if outstanding <= self.outstanding:
                ...
                frame      = SFLFrame()
                frame.cmd  = sfl_cmd_load
                frame_data = f.read(min(remaining, self.length))
                frame.payload = current_address.to_bytes(4, "big")
                #print("Address : 0x",hex(current_address))
                frame.payload += frame_data
                ...
                if self.IR:
                    SPIFLASH_SECTOR_SIZE=65536
                    if( (current_address & ~(SPIFLASH_SECTOR_SIZE - 1)) == current_address):
                        self.delay = 0.4
                    else:
                        self.delay = 0.005
